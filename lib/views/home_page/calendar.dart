...
class Calendar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cellDimension = (MediaQuery.of(context).size.width -
            (cellSpacing * 6) -
            (padding * 2)) /
        7;
    ...
    return StreamBuilder<DateTime>(
        ...
        builder: (context, snapshot) {
          ...
          final daysNumber = DateUtils.getDaysInMonth(
              snapshot.data!.year, snapshot.data!.month);
                      ...
                      child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                              color: Color.lerp(
                                  ...
                                  controller.countHabitsDone(
                                          index - initialWhites) /
                                      controller.countAllHabits(
                                          index - initialWhites)),
              ...
              StreamBuilder<int>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return AnimatedPositioned(
                      ...
                      left: padding +
                          (cellDimension + cellSpacing) *
                              ((selectedDayIndex + initialWhites) % 7),
                      top: padding +
                          (cellDimension + cellSpacing) *
                              ((selectedDayIndex + initialWhites) / 7).floor(),
                      child: SizedBox(
                          width: cellDimension,
                          height: cellDimension,
                          child: CustomPaint(
                            painter: SelectionPainer(
...
class SelectionPainer extends CustomPainter {
  ...
  SelectionPainer({required Color color}) {
    basePaint = Paint()
      ..color = color
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.quadraticBezierTo(
        size.width + OFFSET, -OFFSET, size.width - CURVE, -OFFSET);
    ...
    path.quadraticBezierTo(
        -OFFSET, size.height + OFFSET, CURVE, size.height + OFFSET);
    ...
    path.quadraticBezierTo(size.width + OFFSET, size.height + OFFSET,
        size.width - CURVE, size.height + OFFSET);
