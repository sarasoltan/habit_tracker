...
import 'package:project2/views/full_year_page/full_year_page.dart';
import 'package:project2/dialogs/habits_dialogs/habits_dialog.dart';
...
class HomePageController {
  ...
  int countHabitsDone(int index) {
    if (days.length > index) {
      return days[index]
          .habits
          .keys
          .where((key) => days[index].habits[key] ?? false)
          .length;
    }
    return 0;
  }
  ...
  void showFullYearDialog(BuildContext context) {
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (ctx) => FullYearPage()));
  }
